...
Se puede observar que existen leves discrepancias entre la curva de la simulación y la medición de laboratorio. La mayor diferencia se encuentra en la tensión de foward del LED, la cual es de $\approx 0,2V$.
...
Para este circuito se realizo el analisis de pequeña señal, reduciendolo al siguiente circuito
\begin{figure}[H]
	\centering
	\includegraphics[width=1 \textwidth]{CircEq.PNG}	
	\caption{Analisis de pequeña señak.}
...
Obteniendo los valores de $R_\pi$ = ??(ver whatsapp) Obtenida a partir de la formula \[R_\pi = \frac{V_T}{I_c}\cdot (\beta +1) \]
 obteniendo Ic del spice y calculando $V_T$ $\approx$ 25mV
El valor de $R_0$ fue sacado de la simulación. El valor de $\beta$ fue sacado de la datasheet , dado a que el fabricante da un rango de valores, elegimos un valor de 295 (CREO FIJATE LO QUE DIGA EL WHATSAPP (QUE MANDE DEL SPICE))  
...
Para obtener la frecuencia de corte, se toma el valor de la frecuencia cuando la ganancia es de -3dB, obteniendo \[f_c = f \vert_{|H(f)|=-3dB} \approx 800KHz\]
